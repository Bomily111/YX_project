...
export function createViewer(container: string | HTMLElement): Cesium.Viewer {
  const viewer = new Cesium.Viewer(container, {
    baseLayer: false as any, baseLayerPicker: false, geocoder: false,
...
export function enableOnDemandRender(viewer: Cesium.Viewer) {
 ...
 */
export function installOrbitControls(viewer: Cesium.Viewer, getFocus: () => Cesium.Cartesian3 | undefined) {
  ...
  const onWheel = (delta: number) => {
    ...
    if (delta > 0) { if (dist - step < MIN) step = Math.max(0, dist - MIN); cam.zoomIn(step) }
    else { if (dist + step > MAX) step = Math.max(0, MAX - dist); cam.zoomOut(step) }
    scene.requestRender()
  }
