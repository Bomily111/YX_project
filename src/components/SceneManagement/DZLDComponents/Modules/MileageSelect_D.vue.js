...
// 监听VS/VP切换按钮的变化
watch(selectedVoxelType, (newVal) => {
    if (tunnel_selectedOption.value === 'TSP反演' && Object.keys(selectedAllPoints).length > 0) {
        const currentMileage = Object.keys(selectedAllPoints)[0];
        const row = selectedAllPoints[currentMileage];
        loadmodels(row, DTScopeEngine.viewer, 'TSP反演');
    }
});
// 监听 tunnel_selectedOption 变化
watch(tunnel_selectedOption, (newVal, oldVal) => {
    if (newVal !== oldVal) {
        DTScopeEngine.getViewer(() => {
            const viewer = DTScopeEngine.viewer;
            // 1. 清空所有3DTiles
            const tilesets = viewer.scene.primitives.filter((primitive) => primitive instanceof Cesium.Cesium3DTileset);
            tilesets.forEach((tileset) => {
                viewer.scene.primitives.remove(tileset);
            });
            // 2. 清空体素模型（通过 Previous 类）
            if (pre) {
                pre.clearPrimitiveModel(); // 调用已有的清理方法
            }
            // 3. 清空高亮线和其他实体（可选）
            for (const key in loadedAllPointEntities) {
...
function loadmodels(row, viewer, tunnel_selectedOption) {
    ...
    const modelLoaders = {
        ...
        TSP反演: () => {
            const pathMap = {
                vp: 'data/VP/Vp.raw.json',
                vs: 'data/VS/Vs.raw.json',
                e: 'data/E/E.raw.json',
            };
            const path = pathMap[selectedVoxelType.value] ?? 'data/VS/Vs.raw.json';
            loadVoxel(row, path, viewer);
        },
        瞬变电磁: () => loadVoxel(row, 'data/TEM/TEMRt.raw.json', viewer),
...
let __VLS_29;
const __VLS_30 = ({ selectionChange: {} },
