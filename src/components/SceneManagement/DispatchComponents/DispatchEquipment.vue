<template>
  <div class="dp-float" :style="{ left: pos.x + 'px', top: pos.y + 'px' }">
    <div class="dpf-header" @mousedown="startDrag">
      ...
      <span class="dpf-title">设备调度 · 实时状态</span>
      <span class="dpf-badge">{{ activeCount }}台运行</span>
...
const statusCls   = (s: string) => s === 'working' ? 'st-working' : 'st-idle';
const statusLabel = (s: string) => s === 'working' ? '作业中' : '待机';
</script>
...
.dpf-title { flex: 1; font-size: 12px; font-weight: bold; color: rgba(240, 220, 200, 0.85); }
.dpf-badge { font-size: 10px; color: #ffaa00; background: rgba(255, 170, 0, 0.12); padding: 1px 6px; border-radius: 3px; }
.dpf-close {
...
.resource-item {
  ...
  &.selected { background: rgba(0, 60, 140, 0.6); border-color: #ffaa00; box-shadow: 0 0 8px rgba(255, 170, 0, 0.2); }
}
...
.ri-status {
  ...
  &.st-idle    { background: rgba(100, 100, 100, 0.2); color: #aaa;    border: 1px solid rgba(100, 100, 100, 0.3); }
}
