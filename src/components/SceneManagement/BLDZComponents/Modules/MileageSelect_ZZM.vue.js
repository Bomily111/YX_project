...
//监听里程选择的变化
watch(() => props.targetMileage, (newMileage) => {
    if (newMileage) {
        console.log('收到新的 targetMileage:', newMileage);
        scrollToMileage(newMileage);
    }
}, { immediate: true });
//滚动到指定里程函数
function scrollToMileage(mileage) {
    const targetMeters = mileageToMeters(mileage);
    console.log('目标里程（米）:', targetMeters);
    if (!props.Points || !Array.isArray(props.Points)) {
        return;
    }
    // 查找与 targetMeters 距离差最小的匹配项
    let minDiff = Infinity;
    let index = -1;
    props.Points.forEach((row, i) => {
        const rowMeters = mileageToMeters(row.mileage || row.distance || '');
        if (rowMeters !== null) {
            const diff = Math.abs(rowMeters - targetMeters);
            if (diff < minDiff) {
                minDiff = diff;
                index = i;
            }
    ...
    if (index === -1) {
        console.warn('没有找到可比较的里程数据');
        return;
    }
    if (index === -1) {
        console.warn('没有找到 400 米范围内的里程匹配项');
        return;
    }
    console.log(' 找到匹配行 index:', index, ' 里程:', props.Points[index].mileage);
    nextTick(() => {
        const row = props.Points[index];
        if (tableRef.value) {
            tableRef.value.setScrollTop(index * 40); // 滚动到该行
            tableRef.value.setCurrentRow(row); // 高亮该行（Element Plus 内置）
        }
        lookAtTo(props.Points[index]);
    });
}
console.log('MileageSelect_ZZM组件的值:', props.tunnel_selected);
console.log('MileageSelect_ZZM组件的值:', props.Points);
console.log('目标里程的值:', props.targetMileage);
...
function loadmodels(row, viewer, tunnel_selectedOption) {
    ...
    // 根据选项加载不同模型
    switch (tunnel_selectedOption) {
        case '掌子面素描':
            // 加载掌子面素描的3D Tiles
            loadTiles(basepath['掌子面素描'] + `/${row.mileage}/tileset.json`, viewer);
            break;
        default:
...
const currentPageData = computed(() => {
    ...
    const showdata = props.Points;
    console.log('当前页数据:', showdata);
    return showdata;
});
// 总数据条数
...
onMounted(async () => {
    ...
    tableData.value = data.data
        ...
        .flat();
    console.log(' tableData 处理完成:', tableData.value);
});
...
/** @type {__VLS_StyleScopedClasses['tunnel_select']} */ ;
__VLS_asFunctionalElement1(__VLS_intrinsics.select, __VLS_intrinsics.select)({
    ...{ onChange: ((e) => __VLS_ctx.emit('changeTunnel', e.target.value, __VLS_ctx.tunnelType)) },
    value: (__VLS_ctx.tunnel_selectedOption),
});
__VLS_asFunctionalElement1(__VLS_intrinsics.option, __VLS_intrinsics.option)({});
(props.tunnel_selected);
...
const { default: __VLS_25 } = __VLS_23.slots;
{
    ...
    const [scope] = __VLS_vSlot(__VLS_26);
    let __VLS_27;
    ...
    const __VLS_33 = ({ click: {} },
        { onClick: (...[$event]) => {
                ...
                // @ts-ignore
                [emit, tunnelType, tunnel_selectedOption, currentPageData, selectChange, lookAtTo,];
            } });
