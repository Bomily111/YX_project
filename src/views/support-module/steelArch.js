...
export function generateSteelArch(params) {
...
export async function generateWorkfaceGLB(params) {
    const path = buildArchPath(params);
    const points = path.getPoints(250);
    let bottomY = Infinity;
    for (const p of points)
        bottomY = Math.min(bottomY, p.y);
    const { R1, R2, betaDeg } = params;
    const deltaR = Math.max(0.1, R2 - R1);
...
export async function generateSteelArchGLB(params) {
