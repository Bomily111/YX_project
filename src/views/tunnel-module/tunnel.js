...
export function segmentModelMatrix(i) {
...
let tunnelModels = [];
export async function loadTunnel(viewer, segments = DEFAULT_SEGMENTS) {
    removeTunnel(viewer);
...
export function removeTunnel(viewer) {
...
export function setTunnelVisible(viewer, show) {
    for (const m of tunnelModels)
        m.show = show;
    viewer.scene.requestRender();
}
export function setTunnelTranslucent(viewer, on) {
    for (const m of tunnelModels) {
        m.color = on
