...
let blastModel: any = null

export async function loadBlast(viewer: Cesium.Viewer): Promise<any> {
  if (blastModel) { blastModel.show = true; viewer.scene.requestRender(); return blastModel }
...
export function setBlastVisible(viewer: Cesium.Viewer, show: boolean) {
  if (blastModel) blastModel.show = show
  viewer.scene.requestRender()
}
 ...
 */
export function flyToTunnelOverview(viewer: Cesium.Viewer) {
  ...
  const dir = Cesium.Cartesian3.normalize(
    Cesium.Cartesian3.add(Cesium.Cartesian3.negate(advance, new Cesium.Cartesian3()), lateral, new Cesium.Cartesian3()),
...
function enuHeading(worldDir: Cesium.Cartesian3, center: Cesium.Cartesian3): number {
 ...
 *  persp 透视: 3/4 斜视
 */
export function flyToBlast(viewer: Cesium.Viewer, view: 'persp' | 'front' | 'side' | 'top' = 'persp') {
  const bs = blastModel?.boundingSphere
  if (!bs) return
  const center = bs.center
  ...
  const lateral = Cesium.Cartesian3.normalize(Cesium.Cartesian3.cross(advance, up, new Cesium.Cartesian3()), new Cesium.Cartesian3())
  const neg = (v: Cesium.Cartesian3) => Cesium.Cartesian3.negate(v, new Cesium.Cartesian3())

  // 掌子面正面朝向 -进尺(已开挖/扫描侧); 相机放在 -进尺侧回看即为正视
  let dirFromCenter: Cesium.Cartesian3, pitchDeg: number, range: number
  let headingOverride: number | null = null
  if (view === 'front') {
    dirFromCenter = neg(advance); pitchDeg = -3; range = 24   // 拉远一点
  } else if (view === 'side') {
    dirFromCenter = lateral; pitchDeg = -6; range = 24
  } else if (view === 'top') {
    // 俯视: 相机正上方看下去, 让隧道走向(进尺)朝屏幕上方
    dirFromCenter = up; pitchDeg = -88; range = 30
    headingOverride = enuHeading(advance, center)
  } else {
    // 3/4: -进尺 与 横向 的合方向
    dirFromCenter = Cesium.Cartesian3.normalize(
      Cesium.Cartesian3.add(neg(advance), lateral, new Cesium.Cartesian3()), V)
    pitchDeg = -16; range = 22
  }
