...
/** 将 .dat 文件文本内容解析为散点云 */
export function parseDatToScattered(text, profileIndex) {
    const points = [];
    ...
    for (const line of lines) {
        ...
        if (!Number.isFinite(lineId) || !Number.isFinite(depth) || !Number.isFinite(value))
            continue;
...
/** 单个测点的极坐标→笛卡尔坐标转换 */
function profileToCartesian(profileIndex, lineId, depth) {
    let alphaDeg; // 方位角 (水平面内, 从 +X 轴测量)
    let betaDeg; // 仰俯角 (水平面为 0, 向上为正)
    if (profileIndex === 3) {
...
export function parseAllProfiles(texts) {
    const all = [];
    for (let i = 0; i < 4; i++) {
