 ...
 */
function offsetPoint(entry, heading, sideOffset) {
...
export class MileageRuler {
    ...
    build() {
        ...
        // ── 刻度绘制辅助 ─────────────────────────────────────
        const addTicks = (indices, tickLen, color, zIndex) => {
...
let _ruler = null;
/** 创建（或获取已有）里程刻度尺 */
export function createMileageRuler(viewer) {
    if (_ruler)
        return _ruler;
...
/** 获取当前里程刻度尺实例 */
export function getMileageRuler() {
    return _ruler;
}
/** 销毁里程刻度尺 */
export function destroyMileageRuler() {
    if (_ruler) {
        _ruler.destroy();
        _ruler = null;
    }
