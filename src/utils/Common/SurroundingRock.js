...
// 精灵图 → 3D 体数据
// ─────────────────────────────────────────────────────────────────────────────
function loadVolumeFromSprite(url, xRes, yRes, zRes, tilesPerRow) {
        ...
        for (let y = 0; y < yRes - 1; y++) {
            for (let x = 0; x < xRes - 1; x++) {
                ...
                if (edgeMask === 0)
                    continue;
                // 12 条边的插值顶点
                const edgePts = new Array(12).fill(null);
                const EDGES = [
...
// 构建 Cesium Primitive
// ─────────────────────────────────────────────────────────────────────────────
function buildPrimitive(vertices, normals, indices, modelMatrix) {
    ...
    const geometry = new Cesium.Geometry({
        ...
        boundingSphere: Cesium.BoundingSphere.fromVertices(Array.from(vertices)),
    });
...
let _primitive = null;
export async function loadSurroundingRock(options, customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer) {
        console.warn('⚠️ 围岩模型：Viewer 未初始化');
        return;
    }
    ...
    if (vertices.length === 0) {
        console.warn('⚠️ 围岩模型：等值面为空，请调整 isovalue');
        return;
    }
    ...
    const origin = Cesium.Cartesian3.fromDegrees(options.anchor.lon, options.anchor.lat, options.anchor.height);
    const enu = Cesium.Transforms.eastNorthUpToFixedFrame(origin);
    const rot = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromRotationZ(headingRad), Cesium.Cartesian3.ZERO);
...
export function removeSurroundingRock(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
