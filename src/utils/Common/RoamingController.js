...
// ── 采样航点 ──────────────────────────────────────────────
function sampleWaypoints(interval) {
...
export class RoamingController {
    ...
    /** 飞到当前航点 */
    flyToCurrent(duration) {
        if (!this.viewer || this.currentIdx >= this.waypoints.length - 1) {
            this.stop();
            return;
        }
        const { destination, heading, pitch } = this.computeCameraTarget(this.waypoints[this.currentIdx], this.waypoints[this.currentIdx + 1]);
        this.viewer.scene.camera.flyTo({
            destination,
    ...
    // ── 飞往指定航点（不启动连续漫游） ────────────────────────
    flyToWaypoint(idx, duration = 1.5) {
        if (!this.viewer || idx >= this.waypoints.length - 1)
            return;
        const { destination, heading, pitch } = this.computeCameraTarget(this.waypoints[idx], this.waypoints[idx + 1]);
        this.viewer.scene.camera.flyTo({
            destination,
            orientation: { heading, pitch, roll: 0 },
            duration,
    ...
    /** 跳转到入口 */
    jumpToEntrance() {
        if (!this.viewer || this.waypoints.length === 0)
            return;
        this.currentIdx = 0;
    ...
    /** 跳转到出口 */
    jumpToExit() {
        if (!this.viewer || this.waypoints.length < 2)
            return;
...
/** 全局单例 */
let instance = null;
export function getRoamingController() {
    if (!instance) {
