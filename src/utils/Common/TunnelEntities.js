...
// ── 主入口：创建所有隧道实体 ─────────────────────────────
export function addTunnelEntities(onHotspotClick) {
    const viewer = getViewer();
    if (!viewer)
        return;
...
export function removeTunnelEntities() {
    ...
    _entityIds = [];
    if (viewer._tunnelHotspotHandler) {
        viewer._tunnelHotspotHandler.destroy();
        delete viewer._tunnelHotspotHandler;
    }
}
// ── 设置隧道实体显隐 ──────────────────────────────────────
export function setTunnelEntitiesVisible(show) {
    const viewer = getViewer();
    if (!viewer)
        return;
    _entityIds.forEach(id => {
