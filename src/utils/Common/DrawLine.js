...
let tunnelGlbPrimitives = [];
let windTunnelGlbPrimitives = [];
let designRockGradeData = null;
let designRockGradeEnabled = false;
let designRockGradeLabels = [];
// ── globe 原始状态暂存（enableBlackModelMode 修改前保存，restoreEarthMode 恢复） ──
let _savedGlobeShow = null;
let _savedGlobeLighting = null;
let _savedGlobeBaseColor = null;
let _savedSkyAtmosphereShow = null;
// 风场隧道模型的变换参数（供 WindVectorField 坐标映射使用）
let _windTunnelModelMatrix = null;
let _windTunnelFinalPos = null;
let _windTunnelHeading = 0;
/** 获取风场隧道模型的变换矩阵（ANSYS 局部坐标 → ECEF 世界坐标） */
let _windTunnelCenterlineDist = 0;
let _windTunnelCenterlinePos = null;
export function getWindTunnelTransform() {
    if (!_windTunnelModelMatrix || !_windTunnelFinalPos)
        return null;
    return {
...
// ── 隧道混凝土纹理 ─────────────────────────────────────────
let concreteTextureUrl = null;
let concreteImage = null;
let _tunnelOpacity = 1.0;
function _buildOpacityOnlyShader(opacity) {
...
function _buildConcreteWithOpacityShader(opacity) {
    if (!concreteImage)
        return null;
    return new Cesium.CustomShader({
        uniforms: {
...
function _buildCurrentShader() {
    const opacity = _tunnelOpacity;
    if (opacity >= 1.0 && !concreteImage)
        return null;
    if (concreteImage)
        return _buildConcreteWithOpacityShader(opacity);
...
export function loadConcreteTexture(textureUrl) {
 ...
 */
export function enableTerrainTransparency(customViewer, nearDistance = 400.0, farDistance = 800.0) {
    ...
    globe.translucency.frontFaceAlphaByDistance = new Cesium.NearFarScalar(nearDistance, 0.9, farDistance, 0.9);
    console.log(`[DrawLine] 地形透视已启用（near=${nearDistance}m，far=${farDistance}m，α=0.9）`);
}
/**
 * 关闭地形透视效果，恢复地形完全不透明
 */
export function disableTerrainTransparency(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    const globe = viewer.scene.globe;
 ...
 * 控制隧道中线实体显示/隐藏
 */
export function setCenterLineVisible(show) {
 ...
 */
export function enableBlackModelMode(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
 ...
 */
export function restoreEarthMode(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    const sceneController = viewer.scene;
 ...
 */
export function loadCenterLine(customViewer, isBlackStyle = true, skipZoom = false) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer) {
...
let rebarPrimitives = [];
let secondRebarPrimitives = [];
// 中线采样工具（懒初始化，供各加载函数复用）
let _clinePositions = null;
let _clineCumLen = null;
function getCenterlineUtils() {
    if (_clinePositions && _clineCumLen) {
        const totalLen = _clineCumLen[_clineCumLen.length - 1];
        return { sampleAt: (t) => sampleAtImpl(t, _clinePositions, _clineCumLen, totalLen), totalLen, makeModelMatrix };
    }
    const feature = centerLineData.features?.[0];
    if (!feature)
        return null;
    ...
    return { sampleAt: (t) => sampleAtImpl(t, positions, cumLen, totalLen), totalLen, makeModelMatrix };
}
function sampleAtImpl(t, positions, cumLen, totalLen) {
...
function makeModelMatrix(pos, next, downOffsetZ = -1.25) {
...
/** 加载洞口钢筋网（第一节 23m） */
export function loadRebarMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (rebarPrimitives.length > 0)
        return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        Cesium.Model.fromGltfAsync({ url, modelMatrix })
            .then((model) => { rebarPrimitives.push(viewer.scene.primitives.add(model)); })
            .catch((e) => console.error(`[DrawLine] 洞口钢筋网加载失败:`, e));
    }
    console.log(`[DrawLine] 洞口钢筋网共 ${count} 层（间距${REBAR_SPACING}m，总长${REBAR_TOTAL_LENGTH}m）`);
}
/** 加载二衬钢筋（第二节 44m，从 23m 处开始） */
export function loadSecondRebarMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (secondRebarPrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const url = i % 2 === 0 ? mesh1Url : mesh2Url;
        Cesium.Model.fromGltfAsync({ url, modelMatrix })
            .then((model) => { secondRebarPrimitives.push(viewer.scene.primitives.add(model)); })
            .catch((e) => console.error(`[DrawLine] 二衬钢筋加载失败:`, e));
    }
    console.log(`[DrawLine] 二衬钢筋共 ${count} 层（起始${SECOND_START}m，总长${SECOND_LENGTH}m）`);
}
/** 移除所有支护模型 */
export function removeRebarMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
...
export function setRebarMeshesVisible(show) {
    for (const p of rebarPrimitives)
        p.show = show;
}
/** 控制洞口钢筋网黄色高亮 */
export function setRebarHighlight(enabled) {
    for (const p of rebarPrimitives) {
        p.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 控制二衬钢筋（第二节 44m）显示/隐藏 */
export function setSecondRebarVisible(show) {
    for (const p of secondRebarPrimitives)
        p.show = show;
}
/** 控制二衬钢筋黄色高亮 */
export function setSecondRebarHighlight(enabled) {
    for (const p of secondRebarPrimitives) {
        p.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有二衬钢筋模型 */
export function removeSecondRebarMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of secondRebarPrimitives) {
...
// ── 钢架模型 ────────────────────────────────────────────────
let steelFramePrimitives = [];
/** 加载钢架模型（从 23m 处开始，间隔 1m，总长 44m） */
export function loadSteelFrameMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (steelFramePrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        const modelMatrix = makeModelMatrix(pos, next, 0);
        Cesium.Model.fromGltfAsync({ url, modelMatrix })
            .then((model) => { steelFramePrimitives.push(viewer.scene.primitives.add(model)); })
            .catch((e) => console.error(`[DrawLine] 钢架加载失败:`, e));
    }
    console.log(`[DrawLine] 钢架共 ${count} 榀（起始${STEEL_START}m，间距${STEEL_SPACING}m，总长${STEEL_LENGTH}m）`);
}
/** 控制钢架显示/隐藏 */
export function setSteelFrameVisible(show) {
    for (const p of steelFramePrimitives)
        p.show = show;
}
/** 控制钢架黄色高亮 */
export function setSteelFrameHighlight(enabled) {
    for (const p of steelFramePrimitives) {
        p.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有钢架模型 */
export function removeSteelFrameMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of steelFramePrimitives) {
...
/** 加载中管棚模型（搭在每榀钢架上） */
export function loadPipeShedMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (pipeShedPrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let ri = 0; ri < ringCount; ri++) {
        ...
        for (const theta of thetaList) {
            ...
            Cesium.Model.fromGltfAsync({
                ...
                .then((model) => { pipeShedPrimitives.push({ model: viewer.scene.primitives.add(model), theta }); })
                .catch((e) => console.error(`[DrawLine] 中管棚加载失败:`, e));
        }
    }
    console.log(`[DrawLine] 中管棚共 ${ringCount} 环 × ${thetaList.length} 根，R=${PIPESHED_RADIUS}m`);
}
/** 控制中管棚显示/隐藏 */
export function setPipeShedVisible(show) {
    for (const p of pipeShedPrimitives)
        p.model.show = show;
}
/** 控制中管棚黄色高亮 */
export function setPipeShedHighlight(enabled) {
    for (const p of pipeShedPrimitives) {
        p.model.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有中管棚模型 */
export function removePipeShedMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of pipeShedPrimitives) {
...
/** 加载锚杆模型（梅花状铺设，垂直插入） */
export function loadAnchorMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (anchorPrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let ri = 0; ri < ringCount; ri++) {
        ...
        const place = (theta) => {
            ...
            Cesium.Model.fromGltfAsync({ url, modelMatrix: m })
                .then((mdl) => { anchorPrimitives.push({ model: viewer.scene.primitives.add(mdl), theta }); })
                .catch((e) => console.error(`[DrawLine] 锚杆加载失败:`, e));
        };
...
/** 控制锚杆显示/隐藏 */
export function setAnchorVisible(show) {
    for (const p of anchorPrimitives)
        p.model.show = show;
}
/** 控制锚杆黄色高亮 */
export function setAnchorHighlight(enabled) {
    for (const p of anchorPrimitives) {
        p.model.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有锚杆模型 */
export function removeAnchorMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of anchorPrimitives) {
...
function buildAnchorMatrix(anchorPos, tunnelHeading, theta) {
    // 钢架同位置模型矩阵（+X=侧向, +Z=上）
    ...
    const r90 = Cesium.Matrix3.fromQuaternion(Cesium.Quaternion.fromAxisAngle(tangentCesium, Math.PI / 2));
...
export function loadConduitMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (conduitPrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let ri = 0; ri < ringCount; ri++) {
        ...
        const place = (theta) => {
            ...
            Cesium.Model.fromGltfAsync({ url, modelMatrix: m })
                .then((mdl) => { conduitPrimitives.push({ model: viewer.scene.primitives.add(mdl), theta }); })
                .catch((e) => console.error(`[DrawLine] 小导管加载失败:`, e));
        };
...
export function setConduitVisible(show) {
    for (const p of conduitPrimitives)
        p.model.show = show;
}
export function setConduitHighlight(enabled) {
    for (const p of conduitPrimitives) {
        p.model.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有注浆小导管模型 */
export function removeConduitMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of conduitPrimitives) {
...
export function loadLockAnchorMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (lockAnchorPrimitives.length > 0)
        return;
    const utils = getCenterlineUtils();
    if (!utils)
        return;
    ...
    for (let ri = 0; ri < ringCount; ri++) {
        ...
        for (const theta of [LOCK_ANCHOR_ANGLE, -LOCK_ANCHOR_ANGLE]) {
            ...
            const m = buildAnchorMatrix(p, tunnelHeading, theta);
            Cesium.Model.fromGltfAsync({ url, modelMatrix: m })
                .then((mdl) => { lockAnchorPrimitives.push({ model: viewer.scene.primitives.add(mdl), theta }); })
                .catch((e) => console.error(`[DrawLine] 锁脚锚杆加载失败:`, e));
        }
    }
    console.log(`[DrawLine] 锁脚锚杆共 ${ringCount} 环 ×2，θ=${LOCK_ANCHOR_ANGLE * 180 / Math.PI}°`);
}
export function setLockAnchorVisible(show) {
    for (const p of lockAnchorPrimitives)
        p.model.show = show;
}
export function setLockAnchorHighlight(enabled) {
    for (const p of lockAnchorPrimitives) {
        p.model.color = enabled ? Cesium.Color.YELLOW : Cesium.Color.WHITE;
    }
}
/** 移除所有锁脚锚杆模型 */
export function removeLockAnchorMeshes(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of lockAnchorPrimitives) {
 ...
 * @param customViewer
 */
export function loadGeoModel(url, lon, lat, height, customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return null;
    // 移除上一个地质模型
    if (activeGeoModel) {
        try {
            viewer.scene.primitives.remove(activeGeoModel);
        }
        catch (_) { }
        activeGeoModel = null;
    }
 ...
 * 移除当前地质模型
 */
export function removeGeoModel(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer || !activeGeoModel)
        return;
    try {
        viewer.scene.primitives.remove(activeGeoModel);
    }
...
function applyDesignRockGradeAppearance(model, modelIndex) {
...
function removeDesignRockGradeLabels(viewer) {
...
function addDesignRockGradeLabels(viewer) {
    if (!designRockGradeData)
        return;
...
export async function setDesignRockGradeModelEnabled(enabled, customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    if (enabled && !designRockGradeData) {
        ...
        if (!response.ok)
            throw new Error(`设计围岩分级文件加载失败: ${response.status}`);
...
export async function flyToDesignRockGradeSegment(modelIndex, customViewer) {
    ...
    const end = Cesium.Cartesian3.fromDegrees(endCfg.lon, endCfg.lat, endCfg.height);
    const midpoint = Cesium.Cartesian3.midpoint(start, end, new Cesium.Cartesian3());
    const forward = Cesium.Cartesian3.normalize(Cesium.Cartesian3.subtract(end, start, new Cesium.Cartesian3()), new Cesium.Cartesian3());
    const up = Cesium.Ellipsoid.WGS84.geodeticSurfaceNormal(midpoint, new Cesium.Cartesian3());
    const destination = Cesium.Cartesian3.add(Cesium.Cartesian3.add(midpoint, Cesium.Cartesian3.multiplyByScalar(forward, -240, new Cesium.Cartesian3()), new Cesium.Cartesian3()), Cesium.Cartesian3.multiplyByScalar(up, 110, new Cesium.Cartesian3()), new Cesium.Cartesian3());
...
export function loadTunnelGlb(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    ...
    for (let i = 0; i < SEGMENT_COUNT; i++) {
        ...
        Cesium.Model.fromGltfAsync({ url, modelMatrix })
            ...
            .catch((e) => console.error(`[DrawLine] tunnel${idx}.glb 加载失败:`, e));
    }
 ...
 * 移除所有 GLB 隧道节
 */
export function removeTunnelGlb(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    for (const p of tunnelGlbPrimitives) {
 ...
 * 控制 GLB 隧道节显示/隐藏
 */
export function setTunnelGlbVisible(show) {
    for (const p of tunnelGlbPrimitives)
        if (p)
            p.show = show;
}
export function setTunnelTranslucent(on, customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
 ...
 * 模型是直的，需放在弯曲度最小的位置
 */
export function loadWindTunnelGlb(customViewer, visible = true, translucent = false) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    ...
    Cesium.Model.fromGltfAsync({ url: 'data/wind/suidao.glb', modelMatrix: t.modelMatrix })
        ...
        .catch((e) => console.error('[DrawLine] wind/suidao.glb 加载失败:', e));
}
/**
 * 移除风场模拟隧道模型
 */
export function removeWindTunnelGlb(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
...
/** 控制风场隧道模型显示/隐藏 */
export function setWindTunnelVisible(show) {
    for (const p of windTunnelGlbPrimitives)
        p.show = show;
}
export function setWindTunnelTranslucent(on, customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
...
function _getWindTunnelTransform() {
    const feature = centerLineData.features?.[0];
    if (!feature)
        return null;
    const raw = feature.geometry.coordinates;
    ...
    for (let i = 0; i < dirs.length; i++) {
        let j = i;
        while (j < dirs.length) {
            if (Cesium.Cartesian3.dot(dirs[i], dirs[j]) < ANGLE_THRESHOLD)
                break;
            j++;
        }
...
/** 初始化里程刻度尺（默认隐藏） */
export function loadMileageRuler(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return null;
...
/** 设置里程刻度尺可见性 */
export function setMileageRulerVisible(show) {
