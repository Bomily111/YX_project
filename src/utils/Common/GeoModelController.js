...
// ── 运行时状态 ────────────────────────────────────────────
let tunnelEntity = null;
let glbPrimitives = [];
/** 激活指定 key 的地质模型：加载体数据/GLB + 摆放随道参考 + 飞相机 */
export function activateGeoModel(key, customViewer) {
    const cfg = MODEL_CONFIGS[key];
    if (!cfg)
        return;
    const doActivate = (viewer) => {
        // 清理上一个模型
        _cleanup(viewer);
        ...
        const loadGlb = (url, pos) => {
            ...
            if (glbHeading !== undefined) {
                ...
                if (cfg.glbYRot) {
                    const yRot = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromRotationY(cfg.glbYRot));
                    modelMatrix = Cesium.Matrix4.multiply(modelMatrix, yRot, new Cesium.Matrix4());
                }
                if (cfg.glbZRot) {
                    const zRot = Cesium.Matrix4.fromRotationTranslation(Cesium.Matrix3.fromRotationZ(cfg.glbZRot));
        ...
        if (cfg.glbItems && cfg.glbItems.length > 0) {
            ...
            for (const item of cfg.glbItems) {
                ...
                const pos = [lon, lat2, refPos[2] + (item.heightOffset ?? 0)];
        ...
        if (cfg.volumeUrl) {
            ...
            const orientation = Cesium.Transforms.headingPitchRollQuaternion(pos, new Cesium.HeadingPitchRoll(cfg.tunnelHeading, 0, 0));
        ...
        viewer.scene.camera.flyTo({
            ...
            duration: 1,
            complete: useLookAt ? () => {
                viewer.camera.lookAt(Cesium.Cartesian3.fromDegrees(cfg.lookAtPos[0], cfg.lookAtPos[1], cfg.lookAtPos[2]), new Cesium.Cartesian3(cfg.lookAtOffset[0], cfg.lookAtOffset[1], cfg.lookAtOffset[2]));
            } : undefined,
        });
    };
    if (customViewer) {
...
export function switchTSPLayer(subKey, customViewer) {
    const urlMap = {
...
export function switchTEMLayer(subKey, customViewer) {
    const urlMap = {
...
export function setTunnelModelVisible(show) {
    if (tunnelEntity) {
        tunnelEntity.show = show;
    }
}
// ── 主界面围岩模型（直接叠加在地形上，不切黑底）────────────
let rockPrimitive = null;
/**
 * 在主界面中线上加载围岩 GLB 模型（不切换场景模式，不飞相机）
 */
export function loadRockModel(customViewer) {
 ...
 * 显示 / 隐藏主界面围岩模型
 */
export function setRockModelVisible(show) {
    if (rockPrimitive)
        rockPrimitive.show = show;
}
/**
 * 从场景中移除主界面围岩模型
 */
export function removeRockModel(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer || !rockPrimitive)
        return;
 ...
 */
export function updateRockMatrix(lon, lat, height, headingDeg, pitchDeg, rollDeg) {
    if (!rockPrimitive)
        return;
    const pos = Cesium.Cartesian3.fromDegrees(lon, lat, height);
    const hpr = new Cesium.HeadingPitchRoll(Cesium.Math.toRadians(headingDeg), Cesium.Math.toRadians(pitchDeg), Cesium.Math.toRadians(rollDeg));
    Cesium.Matrix4.clone(Cesium.Transforms.headingPitchRollToFixedFrame(pos, hpr), rockPrimitive.modelMatrix);
}
/** 退出模型视图时清理所有地质模型 */
export function deactivateGeoModel(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
    _cleanup(viewer);
...
function _cleanup(viewer) {
    ...
    glbPrimitives = [];
    if (temGlbPrimitive) {
        viewer.scene.primitives.remove(temGlbPrimitive);
 ...
 */
export function mergeModelConfigsFromApi(instances) {
    for (const inst of instances) {
        ...
        if (inst.volume_url)
            config.volumeUrl = inst.volume_url;
        if (inst.glb_urls?.length)
            config.glbItems = inst.glb_urls;
        if (inst.glb_heading != null)
            config.glbHeading = inst.glb_heading;
        if (inst.glb_y_rot != null)
            config.glbYRot = inst.glb_y_rot;
        if (inst.glb_z_rot != null)
            config.glbZRot = inst.glb_z_rot;
        if (inst.reference_mileage != null)
            config.referenceMileage = inst.reference_mileage;
...
/** 获取当前所有模型配置的 key 列表 */
export function getModelConfigKeys() {
...
let temGlbPrimitive = null;
let temGlbCurrentUrl = '';
export function loadTemAnomalyGlb(customViewer, dataDir) {
    const viewer = customViewer || DTScopeEngine.viewer;
    if (!viewer)
        return;
...
export function unloadTemAnomalyGlb(customViewer) {
    const viewer = customViewer || DTScopeEngine.viewer;
...
export function setTemGlbVisible(show) {
    if (temGlbPrimitive) {
