...
import { lerpColor } from './WindFieldSimulation';
// ── 数据采样 ──────────────────────────────────────────────
const SAMPLE_STRIDE = 4; // 每 N 个网格单元取 1 个箭头
// ── 渲染状态 ──────────────────────────────────────────────
let vectorPrimitive = null;
let allCells = [];
...
function parseCSV(text) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (isNaN(x) || isNaN(y) || isNaN(z) || isNaN(magnitude))
            continue;
 ...
 */
function makeConeGeom(baseRadius) {
...
function buildArrowInstances(cells, modelMatrix) {
    const instances = [];
    ...
    for (let i = 0; i < cells.length; i += SAMPLE_STRIDE) {
        ...
        const worldPos = Cesium.Matrix4.multiplyByPoint(modelMatrix, localPos, new Cesium.Cartesian3());
        // 局部方向 → 世界方向
        Cesium.Cartesian3.normalize(lv, lv);
        const worldDir = Cesium.Matrix4.multiplyByPointAsVector(modelMatrix, lv, new Cesium.Cartesian3());
        ...
        if (Cesium.Cartesian3.magnitude(cross) < 1e-10) {
            ...
            rotation = Cesium.Matrix3.fromQuaternion(Cesium.Quaternion.fromAxisAngle(cross, Math.acos(Cesium.Math.clamp(dot, -1, 1))), new Cesium.Matrix3());
        }
        const rotTranslate = Cesium.Matrix4.fromRotationTranslation(rotation, worldPos);
        // 尺寸
        ...
        const colorAttr = Cesium.ColorGeometryInstanceAttribute.fromColor(new Cesium.Color(r, g, b));
        // ── 杆身（圆柱），位于 Z∈[-totalLen/2, -totalLen/2+shaftLen] ──
        const shaftScale = Cesium.Matrix4.fromScale(new Cesium.Cartesian3(1, 1, shaftLen));
        const shaftOffset = Cesium.Matrix4.fromTranslation(new Cesium.Cartesian3(0, 0, -totalLen / 2 + shaftLen / 2));
        ...
        const headScale = Cesium.Matrix4.fromScale(new Cesium.Cartesian3(1, 1, headLen));
        const headOffset = Cesium.Matrix4.fromTranslation(new Cesium.Cartesian3(0, 0, +totalLen / 2 - headLen / 2));
...
// ── 公共 API ──────────────────────────────────────────────
/** 开启矢量场可视化 */
export async function startVectorField(viewer) {
...
/** 清除矢量场 */
export function removeVectorField(viewer) {
    const v = viewer ?? DTScopeEngine.viewer;
    if (!v || !vectorPrimitive)
        return;
    try {
