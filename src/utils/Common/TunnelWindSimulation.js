...
let field = null;
let computedPaths = [];
let staticPrimitive = null;
let tracerCollection = null;
let tracers = [];
...
function sample(x, y) {
    if (!field)
        return { axial: 1, lateral: 0, vertical: 0, potential: 0 };
    const cellW = TUNNEL_WIDTH / NX;
    const cellH = TUNNEL_HEIGHT / NY;
...
function seedPoints() {
    ...
    for (let li = 0; li < SEED_LANES; li++) {
        for (let la = 0; la < SEED_LAYERS; la++) {
            ...
            const y = (la + 0.5) * dh;
            if (Math.hypot(x - DUCT_CX, y - DUCT_CY) < DUCT_R)
                continue;
            seeds.push({ x, y });
        }
    }
...
function cumulativeLength(points) {
    const cumLen = new Array(points.length);
    cumLen[0] = 0;
...
function samplePathAtDistance(points, cumLen, d, result) {
...
function sampleSpeedAtDistance(speeds, cumLen, d) {
    const total = cumLen[cumLen.length - 1];
    d = Math.max(0, Math.min(total, d));
    let lo = 0;
    let hi = cumLen.length - 1;
...
function destroyTracers(viewer) {
    if (preRenderListener) {
...
function onPreRender() {
    if (!flowEnabled || !tracerCollection)
        return;
    ...
    for (const t of tracers) {
        ...
        const c = t.polyline.material.uniforms.color;
...
// ── 公共 API ──────────────────────────────────────────────
/** 开启程序化模拟风场 */
export function startProceduralWind(viewer) {
    ...
    for (const s of seeds) {
        ...
        if (path.peakDisturbance < DISTURBANCE_THRESHOLD)
            continue;
        smooth(path);
        const worldPoints = [];
...
/** 清除模拟风场 */
export function removeProceduralWind(viewer) {
    ...
    if (staticPrimitive) {
        try {
            v.scene.primitives.remove(staticPrimitive);
        }
        catch (_) { /* ignore */ }
...
/** 流动（拖尾）开关 */
export function setProceduralFlow(on) {
    flowEnabled = on;
    if (tracerCollection)
        tracerCollection.show = on;
    if (on && computedPaths.length) {
        const viewer = DTScopeEngine.viewer;
        if (viewer)
            ensureTracers(viewer);
    }
}
/** 调节显示强度 (0-11)，实时重建静态流线宽度 */
export function changeProceduralPower(power) {
    const newPower = Math.max(0, Math.min(11, Math.floor(power)));
    if (newPower === currentPower)
        return;
    currentPower = newPower;
    const viewer = DTScopeEngine.viewer;
    if (!viewer || !staticPrimitive || !computedPaths.length)
        return;
    removeProceduralWind(viewer);
    renderStatic(viewer);
    if (flowEnabled)
...
/** 获取当前强度 */
export function getProceduralPower() {
    return currentPower;
}
/** 重置缓存（几何/参数变化后调用） */
export function resetProceduralWind() {
    field = null;
    computedPaths = [];
}
