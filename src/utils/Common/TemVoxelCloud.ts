...
let voxelPrimitive: any = null

function kColor(k: number): Cesium.Color {
  if (k < 570) return Cesium.Color.fromCssColorString('#4488ff')
  if (k < 590) return Cesium.Color.fromCssColorString('#44ccff')
  if (k < 610) return Cesium.Color.fromCssColorString('#44dd66')
  return Cesium.Color.fromCssColorString('#ddaa00')
}

function buildModelMatrix(): Cesium.Matrix4 {
...
export async function loadTemVoxelCloud(dataDir?: string) {
  ...
  for (let i = 1; i < lines.length; i += stride) {
    ...
    if (isNaN(x) || isNaN(y) || isNaN(z) || isNaN(k)) continue
...
export function unloadTemVoxelCloud() {
  ...
  if (voxelPrimitive && viewer) {
    try { viewer.scene.primitives.remove(voxelPrimitive) } catch {}
    voxelPrimitive = null
  }
}
