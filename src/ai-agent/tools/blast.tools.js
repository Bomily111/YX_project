        ...
        executor: async (input) => {
            try {
                ...
                if (!designs.length)
                    return '📋 暂无数据可供分析';
                // 找到相关设计
                const relevant = dk ? designs.filter((d) => d.dk_number?.includes(dk)) : designs;
                if (!relevant.length)
                    return `📋 未找到 ${dk ? `里程 ${dk} 的` : ''}爆破设计数据`;
                const d = relevant[0];
